...
import type { Grade, Lesson, StudentProgress } from "@/lib/types";
import { useAuth } from "@/hooks/use-auth";
import { getStudentProgress } from "@/lib/firestore";
import { Loader2 } from "lucide-react";

export function GradeDashboardClient({ 
    grade, 
    allLessons,
    gradeLessons, 
  ...
  useEffect(() => {
    // If the user logs in or out, we need to refetch their progress.
    async function fetchProgress() {
        if (user) {
            ...
            // If user logs out, clear their progress
            setStudentProgress([]);
        }
    }
    if (!authLoading) {
